...
MonthPlot_18_21_Rolling <- month_data_Rolling %>% 
  dplyr::filter(as.numeric(Year) >= 2018) %>%
  dplyr::filter(as.numeric(Year) <= 2021) %>%
  ggplot(aes(Month)) + 
  geom_segment(aes(xend = Month, y = MonthMean, yend = MonthMeanFit)) + 
  geom_point(aes(y = MonthMean, color = "Actual"), size = 4) +
  geom_point(aes(y = MonthMeanFit, color = "Prediction"), size = 4) +
  facet_grid(~Year, space = "free_x", scales = "free_x", switch = "x") + 
  scale_color_manual(values = c("Train" = "darkgrey", "Test" = "red", "Prediction" = "blue", "Actual" = "red")) +
  theme_minimal() + 
...
ggsave("MonthPlot_18_21_Rolling.jpg", plot = MonthPlot_18_21_Rolling, device = "jpg", dpi = 300, width = 12, height = 6)

MonthPlot_Both <- ggarrange(MonthPlot_18_21_Rolling, MonthPlot_Rolling, 
          labels = c("A", "B"), 
          ncol = 1, nrow = 2)

ggsave("MonthPlot_Both.jpg", plot = MonthPlot_Both, device = "jpg", dpi = 300, width = 12, height = 10)
...
#T-Test plot - if you update the analysis, you'll need to rebuild this dataframe!!
...
tsummary <- data.frame(
  Period = factor(c("Pre-Roadwork Test", "Roadwork Test"), levels = c("Pre-Roadwork Test", "Roadwork Test")),
  Average = c(-0.807, -23.5),
  Lower = c(-3.58, -25.5),
  Upper = c(1.96, -21.49),
  P = c(0.567, 2.20e-16)
)

# Add a factor for significance based on P value
tsummary$Significance <- factor(ifelse(tsummary$P < 0.05, "Significant", "Non-significant"),
                            levels = c("Non-significant", "Significant"))

# Plot
T_Test_Plot <- ggplot(tsummary, aes(x = Period, y = Average, ymin = Lower, ymax = Upper, color = Significance)) +
  ...
  theme_minimal() +
  labs(x = "Period",
       y = "Average Predicted minus Observed") +
  theme(legend.title = element_blank(), legend.position = "right")

ggsave("T_Test_Plot.jpg", plot = T_Test_Plot, device = "jpg", dpi = 300, width = 6, height = 5)
```
...
Rolling_p_values_uncorrected <- c(Rolling_22_23$p.value, Rolling_18_21$p.value)

p_adjust_year_lags <- p.adjust(YearLag_p_values, method = "BH")
print(p_adjust_year_lags)

p_only_rolling_corrected <- p.adjust(Rolling_p_values, method = "BH")
